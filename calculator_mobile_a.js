...
var SQRTPI="1.";
var SQRT2="1."; 
    ...
    var str_af = str_1.substr(dg_index+1 ,star_str_lg-dg_index-1);  
      ...
      var nub_point= m_str_char(part_bf,".") ;  
       ...
       var part_bf_lg = part_bf.length;
                   ...
                   if(part_bf_b1==""){ var part_bf_b1 =1 ;}                                 //20200718
                   var part_bf_b = m_mtx_real_div(part_bf_b1 ,360 );      
                   var part_bf_b  = m_str_e_to_str(part_bf_b);      
                 if(part_bf_b.length >250 ){
                   var part_bf_b  =   m_fix(part_bf_b,250);       //20200813  
                                           }
...
function m_pi_dg_bf_del_1(str){       
 ...
 function m_str_div_single(y,x){      
    var y_a=y;
    var x_a=x;
    var ans_1="a";
    if(y_a==0){ var ans_1=0; return  ans_1; }
    if(x_a==1){ var ans_1=y_a; return  ans_1; }
    if(x_a==y_a){ var ans_1=1;  return  ans_1; }
    var nub_pint_y = m_str_spc_count(y_a,".");   
    var nub_pint_x = m_str_spc_count(x_a,".");  
        if(nub_pint_y==0){var y_a=y_a+".";}     
        if(nub_pint_x==0){var x_a=x_a+".";}
    var dif_plg_y = y_a.length - x_a.length;
    var dif_plg_x = x_a.length - y_a.length;
      if(dif_plg_y >=0 && dif_plg_y <=14){                   
         var x_a = x_a.toString().replace(/\./g,'');                   
         var y_a = y_a.toString().replace(/\./g,'');                    
            for(var i=0 ;i< dif_plg_y ; i++){
              x_a=x_a+"0" ; }                                         
                       }                 
       else if(dif_plg_x >=0 && dif_plg_x <=14) {
         var x_a = x_a.toString().replace(/\./g,'');                    
         var y_a = y_a.toString().replace(/\./g,'');                    
            for(var i=0 ;i< dif_plg_x ; i++){
               y_a=y_a+"0" ; }                                        
                    }                
       else{ ans_1="a" ; return  ans_1;}  
           var x_a =m_mtx_trim(x_a) ;               
           var y_a =m_mtx_trim(y_a) ;               
        var AA= m_mtx_prime_table();         
       if( y_a.length<=14 && x_a.length<=14){       
       for (var i=0;i<AA[0].length ;i++){
         var data_c = parseInt(AA[0][i]) ;   
               for (var j=0;j<2 ;j++){   
                var y_remainder = parseInt(y_a) % data_c ;
                var x_remainder = parseInt(x_a) % data_c ;
                    if(y_remainder==0 && x_remainder==0  ){    
                      var  y_a = parseInt(y_a) / parseInt(data_c);
                      var  x_a = parseInt(x_a) / parseInt(data_c);
                      var   j=0;                        }
                      else{ var j=2 ;}  
                                        }  
           if(data_c >y_a || data_c > x_a || y_a==1 || x_a==1){   
               var i = AA[0].length+1;
              } 
                                              } 
                ...
                var y_a =m_mtx_trim(y_a) ;               
               if(x_a==1){ var ans_1=y_a; return  ans_1; }   
              if(x_a < 501 && x_a.length<=3){
                      var BB= m_mtx_unit_inv_lg_100_table();     
                      var x_a_inv = m_mtx_row_table(BB,x_a); 
                      var x_a_inv = x_a_inv.toString().replace(/\,/g,'');                    
                      var x_a_inv = "0."+ x_a_inv.toString().substr(0,x_a_inv.length);
                      var ans_1 = m_mtx_real_mul(y_a,x_a_inv); 
                         }
              else{ ans_1="a" ; return  ans_1;}
                  var ans_1 =m_mtx_trim(ans_1) ;               
                  var ans_1 = m_str_e_to_str(ans_1);      
    return  ans_1;
  }
...
function m_sin(x){                         
    ...
    var xx_test_nub_e =m_str_power(xx_test_small,0);             
    var xx_test_st = m_str_power(xx_test_small,1);               
      if(m_mtx_real_str_comp(xx_test_nub_e,-500)==2 ){ return  xx;}
    ...
    var data_cle =m_fix(data_cle,500);
     ...
     if(data_cle== 0.125 ){ var ans_0 = SQRT2 ;  var ans_a = m_mtx_real_div( ans_0,2);  if( cle_fst =="-"){ var ans_a = "-"+ans_a;}  return ans_a ;   } 
    var flag = 1;
    var data_a =0;
    var data_nub =0;
    var  data_cle_comp_25 = m_mtx_real_str_comp(data_cle,0.25);    
    var  data_cle_comp_50 = m_mtx_real_str_comp(data_cle,0.50);    
    var  data_cle_comp_75 = m_mtx_real_str_comp(data_cle,0.75);    
    var  data_cle_comp_100 = m_mtx_real_str_comp(data_cle,1);    
...
function m_sin_sum(x,nub){          
  ...
  var  data_xx_comp_0 = m_mtx_real_str_comp(xx,0);             
   ...
   var xx_fst = xx.substr(0,1);
      if(xx_fst =="-"){ var xx = xx.substr(1,xx.length-1); }  
   var idx_e = xx.indexOf("e");
      if( idx_e >0){  var xx_bf = xx.substr(0,idx_e); }
      else{ var xx_bf = xx;}                            
   var xx_bf_p = xx_bf.indexOf(".");               
      if(xx_bf_p >= 1){                         
        var xx_bf = xx_bf.replace(/\./g , "");         
   ...
   if( shif_all >= 2000 &&  shif_all <= 49500) {   
    ...
    var xx_st_1 = m_mtx_real_mul(pi2inv,xx_bf);  
        ...
        xx_st_1=xx_st_1.toString();    
    var xx_st_p = -1;  //init
    var xx_st_p = xx_st_1.indexOf(".");
    var xx_st_1_fst = xx_st_1.substr(0,1);
      if(xx_st_p == 1 && xx_st_1_fst ==0){
        var xx_st_2 = xx_st_1; }
      else if (xx_st_p == -1 && xx_st_1_fst !=0) {  var xx_st_2 = 0; }  
      else if (xx_st_p == -1 && xx_st_1_fst ==0) {  var xx_st_2 = 0; }  
      else{ var xx_st_2 = "0"+xx_st_1.substr( xx_st_p,xx_st_1.length-xx_st_p) ;}  
      var xx_st_2= m_fix(xx_st_2,1500); 
   ...
   if(nub_a==0){ var ans_1= xx_st_2;}   
    ...
    else if(nub_a==2){ var ans_1= ans_1;} 
    else {var ans_1= ans_1;}
   return  ans_1 ;
}

function m_sin_small(x){         
    var xx=x.toString().trim();
    var xx =m_mtx_trim(xx) ;         
    var zz=xx;
    var x_fst=zz.substr(0,1);
     ...
     if( zz_m >= 6 && zz_m != 0){
        ...
        if(zz.length > 1000){
               var   zz_cp = m_fix(zz,1000);      
               var  PIMUL2_cp = m_fix(PIMUL2,1000);
                              }
               else{
                 var   zz_cp = zz;            
         ...
         var data_comp_zz_PIMUL2 = m_mtx_real_str_comp(zz_cp,PIMUL2_cp);   
              ...
              if(zz.length > 500){
               ...
               var  PIMUL2_cp = m_fix(PIMUL2,500);
                              }
           if( zz >= 6 ){
             var data_comp_zz_PIMUL2 = m_mtx_real_str_comp(zz_cp,PIMUL2_cp);   
    ...
    if(zz.length > 1000){
                  var   zz_cp = m_fix(zz,1000);        
                  var PIDIV2_cp = m_fix(PIDIV2,1000);
                  var      PI_cp=m_fix(PI,1000);
                  var PIDIV2MUL3_cp=m_fix(PIDIV2MUL3,1000);
                              }
                  else{
                  var   zz_cp = zz;      
    ...
    var data_comp_zz_PI = m_mtx_real_str_comp(zz_cp,PI_cp);
    var data_comp_zz_PIDIV2MUL3 = m_mtx_real_str_comp(zz_cp,PIDIV2MUL3_cp);
    var data_comp_zz_PIMUL2   =  m_mtx_real_str_comp(zz_cp,PIMUL2_cp);
    ...
    if( data_comp_zz_0==3 ){ ans_1 = 0 ; return ans_1 ;}  
         ...
         var sum_tt = zz;
    var data_comp_sum_tt_PI = m_mtx_real_str_comp(sum_tt,PI);         
    var data_comp_sum_tt_PIMUL2 = m_mtx_real_str_comp(sum_tt,PIMUL2);
      if(data_comp_sum_tt_PIMUL2 ==2 && data_comp_sum_tt_PI==1){                    
...
function m_2pi_rem(x){        
   var x=x.toString().trim();
    ...
    var FF=m_mtx_point_in_int(FF,zz_m);         
    var EE_0 = m_mtx_row_table(AA,0);     
               BB_1[0][i]=0 ;   
    ...
    var tt=1;
        ...
        if(comp_zz_PIDIV2 ==1  && comp_zz_PI !=1){  
          zz = m_mtx_real_sub(PI,zz);   
                ...
                var comp_e_zz_0 = m_mtx_real_str_comp(zz,0);  
    if((comp_e_zz_0 ==3)||(comp_e_zz_500 !=1)){                
              return  zz ;}  
         zz = m_str_e_to_str(zz);      
    var zz_m = m_nub_m(zz) ;           
    var zz_p = m_nub_p(zz) ;          
   ...
   var data_msg =0;
    ...
    var cle_fst = data_cle.toString().substr(0,1);
     if(cle_fst =="-"){ var data_cle = data_cle.toString().substr(1,data_cle.length-1);}  
      ...
      var data_cle_0 = m_mtx_real_str_comp(data_cle,0);          
    var data_cle_125 = m_mtx_real_str_comp(data_cle,0.125);     
    var data_cle_250 = m_mtx_real_str_comp(data_cle,0.25);     
    var data_cle_375 = m_mtx_real_str_comp(data_cle,0.375);     

    var data_cle_500 = m_mtx_real_str_comp(data_cle,0.50); 
    var data_cle_525 = m_mtx_real_str_comp(data_cle,0.525);    
    
    var data_cle_750 = m_mtx_real_str_comp(data_cle,0.75);
    var data_cle_875 = m_mtx_real_str_comp(data_cle,0.875);    
    var data_cle_1000 = m_mtx_real_str_comp(data_cle,1);

     if(data_cle_0== 3  ||data_cle_1000 == 3){ var ans_a = 1;  alert(" ans_a="+ ans_a)   ;return ans_a ; }  
     if(data_cle_500== 3 ){ var ans_a = -1;  return ans_a ; }
     if(data_cle_250== 3 || data_cle_750== 3){ var ans_a = 0;  return ans_a ;   }   
     if(data_cle_125== 3 || data_cle_875== 3){                                     
                   var ans_0 = SQRT2 ;                     
                   var ans_a = m_mtx_real_div( ans_0,2);
                       return ans_a ;   } 
     if(data_cle_375== 3 ||data_cle_525 == 3){               
                   var ans_0 = SQRT2 ;                     
                   var ans_a = m_mtx_real_div( ans_0,2);
                   var ans_a ="-"+ans_a;
                       return ans_a ;   }    
    ...
    var ans_0 = m_mtx_real_mul(zz_s,PIMUL2);            
       if(flag_a==1){  var ans_a = m_sin_matrix_1(ans_0);}      
        ...
        MTX_COL = data_t_MTX_COL_1; 
   if(flag == 2 || flag == 3){ var ans_a="-"+ans_a;}
    return ans_a ;
}
...
function m_tan(x){                           
  ...
  var cle_fst = data_cle.toString().substr(0,1);
     ...
     if(data_cle== 0.125 ||data_cle== 0.625){
                   var ans_a = 1 ;                     
                   if(cle_fst =="-"){ var ans_a =-1 ;}
                       return ans_a ;   } 
     if(data_cle== 0.375 ||data_cle== 0.875){
                   var ans_a = -1 ;                    
                  if(cle_fst =="-"){ var ans_a =1 ;}
                     return ans_a ;   } 
   ...
   if(flag_a==0){     
     var ans_1= m_mtx_real_div(ans_s_1,ans_c_1);}
   else { var ans_1= m_mtx_real_div(ans_c_1,ans_s_1);}
        MTX_COL = data_t_MTX_COL_1; 
    if(cle_fst =="-" && (flag == 1 || flag == 3)){ var ans_1="-"+ans_1;}
    if(cle_fst !="-" && (flag == 2 || flag == 4)){ var ans_1="-"+ans_1;}
  return ans_1 ;    
   }
 ...
 var ans_t = 0;  
  ...
  var  data_xx_comp_0 = m_mtx_real_str_comp(xx,0);    
      ...
      if ((xx == "1" || xx == 1) && xx.length==1 ) {  var ans_t = 0 ; return ans_t;}
       var xx_str =""; 
       var xx_str = m_s_e_ln(xx,0);       
       var nub_e =  m_s_e_ln(xx,1); 
       var sum_ta = m_ln_small(xx_str);    
...
function m_ln_small(x){       
 var xx = x.toString().trim();
  var xx = m_str_e_to_str(xx);  //20200827       
  var xx_st=xx; 
  var sum_tt = 0 ;
  var nub_e=0;  
      FF=m_new_zero_mtx(1,MTX_COL);            
     ...
     var xx_st = m_str_e_to_str(xx_st);         
         ...
         SUM_67=m_mtx_cell_five(SUM_67);    
             if( xx_st !='1'  ){                   
              ...
              var sum_tt = sum_tt.toString();      //20200827
              var sum_fst=sum_tt.substr(0,1);
             var data_comp_xx_1  =  m_mtx_real_str_comp(xx,1); //20200827
...
function m_asin(x){                                              
  ...
  var data_comp_zz_1_ag   =  m_mtx_real_str_comp(zz,1);       
   var data_comp_zz_0_ag   =  m_mtx_real_str_comp(zz,0);           
   if(data_comp_zz_0_ag==3){ var sum_total= 0; return sum_total; }        
   if(data_comp_zz_1_ag ==3){ var sum_total= PIDIV2; 
   if(zz==0){ var sum_total= 0; return sum_total; }        
   if(zz==1){ var sum_total= PIDIV2; 
             
        if(flag_a==1){sum_total="-"+sum_total;}
           return sum_total; }   
   var zz_fix = m_fix(zz,1000);
   var data_test1_fix = m_fix(data_test1,1000);    
  ...
  var comp_xx_0 = m_mtx_real_str_comp(xx,0);      
   ...
   var comp_xx_06 = m_mtx_real_str_comp(xx,0.6); 
    ...
    if( comp_xx_06 !=1 || comp_xx_17 !=2){           
         ...
         var ans_1 =  m_atan_part_center(xx);}
    if(xx_fst=="-"){ var ans_1 = "-"+ans_1;}                    
      var ans_1 = ans_1.toString().trim();      
   return ans_1;
...
function m_atan_part_outer(x){    
    ...
    var sum_tt =0 ;
    var tt=1;
    var comp_zz_17 = m_mtx_real_str_comp(zz,1.7);            
    if(zz==0){ var sum_total= 0; return sum_total; }        
    if(zz==1){ var sum_total= PIDIV4; return sum_total; }   
    if(comp_zz_17 !=2){     
       var zz=m_mtx_real_div(1,zz); 
       var flag_b=1;}
       var zz = m_str_e_to_str(zz);          
       var zz =m_fix(zz,250);                    
       var comp_zz_055 = m_mtx_real_str_comp(zz,0.55);             
       ...
       var comp_zz_001 = m_mtx_real_str_comp(zz,0.01);     
       var comp_zz_0001 = m_mtx_real_str_comp(zz,0.001);   
      if(comp_zz_0001 !=1  ){ var nn=85 ;}
      else if(comp_zz_001 !=1 && comp_zz_0001 ==1 ){ var nn=130 ;}
      else if(comp_zz_01 !=1 && comp_zz_001 ==1 ){ var nn=250 ;}                         
      else if(comp_zz_015 !=1 && comp_zz_01 ==1  ){ var nn=300 ;}
      else if(comp_zz_025 !=1 && comp_zz_015 ==1 ){ var nn=400 ;}
      else if(comp_zz_035 !=1 && comp_zz_025 ==1 ){ var nn=500 ;}
      else if(comp_zz_045 !=1 && comp_zz_035 ==1 ){ var nn=630 ;}
      else if(comp_zz_055 !=1 && comp_zz_045 ==1 ){ var nn=830 ;}
      else { var nn=999 ;}
        var BB = m_mtx_unit_inv_lg_100_table();     
        var comp_zz_06 = m_mtx_real_str_comp(zz,0.6);             
      if(comp_zz_06 !=1  ){               
        var AA  = m_str_to_mtx(zz) ;              
        var XX = AA;  
        var AA2 = m_mtx_point_mul(AA,AA) ;
             for(var i = 1 ; i < nn+1 ; i=i+2){       
              var BB_i = m_mtx_row_table(BB,i);          
               if(i ==1){
                 var SS = AA;     
                 var XX = AA;}                      
       ...
       if(zz==1){ var sum_total= PIDIV4; return sum_total; }   
        if(comp_zz_1 ==1){    
         ...
         if(comp_zz_1 ==2){        
            var data_a = m_mtx_real_mul(zz,zz);   
            var data_b = m_mtx_real_sub(1,data_a); 
            var data_c = m_mtx_real_div(zz,data_b); 
            var data_d = m_mtx_real_mul(2,data_c); 
            var zz=data_d ;
            var flag_c=1;
            }
            var zz = m_str_e_to_str(zz);          
            var zz =m_fix(zz,1000);                  
            var sum_total_s = m_atan_part_outer(zz);    
          if(flag_c ==1){    
             var sum_total = m_mtx_real_div(sum_total_s,2);
                             }
          if(flag_b ==1){    
              var sum_total = m_mtx_real_sub(PIDIV2 ,sum_total);
                             }
 ...
 var xx_test_st = m_str_power(xx_test_small,1);          
  ...
  var xx_fst = xx.substr(0,1);
   ...
   var comp_xx_n1 = m_mtx_real_str_comp(xx,-1);    
      ...
      if( comp_xx_0 == 3 ){ var ans_1 = 0  ; return ans_1 ;}  
   var comp_xx_1 = m_mtx_real_str_comp(xx,1); 
    var data_test = m_mtx_real_sub('1','1e-100');  
...
function m_str_e_to_str_point(str){     
  ...
  var pos_pot  = yy_st_af.indexOf(".");                  
   ...
   var data_tran = MTX_COL;                      
        if( MTX_COL >100){ MTX_COL =100;  }          
   var yy_st_a = m_pow(10,nub_e_y);  
           MTX_COL = data_tran ;               
    var yy_st_b= m_mtx_real_mul(yy_st,yy_st_a);
                 //alert("yy_st_b =" +yy_st_b);           
 return yy_st_b;   
