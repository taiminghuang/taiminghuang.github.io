...
function m_cos(x){          
    ...
    var data_cle =m_fix(data_cle,250);
     ...
     if(data_cle== 0.125 || data_cle== 0.875){
                   var ans_0 = SQRT2 ;                     //m_pow(2 ,0.5);
                   var ans_a = m_mtx_real_div( ans_0,2);
                       return ans_a ;   } 
     if(data_cle== 0.375 ||data_cle== 0.525){
                   var ans_0 = SQRT2 ;                     //m_pow(2 ,0.5);
                   var ans_a = m_mtx_real_div( ans_0,2);
                   var ans_a ="-"+ans_a;
                       return ans_a ;   } 
...
var xx = x.toString().trim();
   ...
   var xx_fst = xx.substr(0,1);
    ...
    var comp_xx_1 = m_mtx_real_str_comp(xx,1e-100);       
      if(comp_xx_1!=1){
         var ans_1 =xx;
      if(xx_fst=="-"){ var ans_1 = "-"+ans_1;}  
           return ans_1;   }
...
function m_str_e_to_str_point(str){     
  ...
  var st_lg = yy_st.length;
      ...
      var nub_e_y=0; //inition  y
          if(  pos_e != -1){ 
            nub_e_y =m_str_power(yy_st,0);         
            yy_st = m_str_power(yy_st,1);          
                           }
      var nub_e_y_fst = nub_e_y.substr(0,1);
      var pos = nub_e_y.indexOf(".");    
      var nub_e_y_bf=nub_e_y.substr(0,pos); 
      var nub_e_y_af=nub_e_y.substr(pos+1,nub_e_y.length-1-pos);
      if(  pos == -1  ){   return str; }               
      var nub_e_y_af="0."+nub_e_y_af.toString();
      if(nub_e_y_fst =="-"){ 
                     var nub_e_y_af = m_mtx_real_sub(1,nub_e_y_af);     
                     var yy_st = m_mtx_real_div(yy_st,10); } 
      var data_tran = MTX_COL;                          
            if( MTX_COL >100){ MTX_COL =100;  }
      var yy_st_a=m_pow(10,nub_e_y_af);
       MTX_COL = data_tran ;
      var yy_st_b= m_mtx_real_mul(yy_st,yy_st_a);
      var yy_st_b = m_str_e_to_str(yy_st_b); 
      var yy_st_c= yy_st_b +"e"+nub_e_y_bf;
 return yy_st_c;
...
function m_mtx_real_div(y,x){    
     ...
     var y_msg = m_mtx_test_msg(y);
        if(x_msg !=-1 ){ return x;}     
        if(y_msg !=-1 ){ return y;}
        if((x==0 || x=="0")&& (x.length==1)){  return message_1(29); }
   ...
   var data_y_lg=yy_st.length;
     ...
     var BB = m_str_to_mtx(xx_st) ;
     var yy_stdivxx_st =""; 
     var yy_stdivxx_st =  m_str_div_single(yy_st,xx_st);       
      if(yy_stdivxx_st !="a"){
             var CC = m_str_to_mtx(yy_stdivxx_st) ;}   
      else {
         if(yy_st==xx_st){
             var CC = m_str_to_mtx(1) ;}     
         else if(xx_st==1){
             var CC = m_str_to_mtx(yy_st) ;}     
         else{
